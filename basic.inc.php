...
if($bankid>0) {
	...
	$query = DB::query("SELECT opstatus,opnum,extchar,begintime,endtime FROM ".DB::table('plugin_bankoperation')." WHERE bankid='$bankid' AND uid='$uid' AND optype='0'");
	...
	if($action=='open') {
		...
		if($mycash<$bankinfo['opencost']) {
			...
			if($lognum>0) {
				DB::query("UPDATE ".DB::table('common_member_count')." SET $moneycredits=$moneycredits-$lognum WHERE uid='$uid'");
			}
			DB::query("INSERT INTO ".DB::table('plugin_bankoperation')."(uid,username,bankid,optype,opstatus,opnum,extchar,begintime,endtime) VALUES('$uid','$username','$bankid','0','0','0','$bankpass','$timestamp','$timestamp')");
		...
		if($op=='in') {
			...
			DB::query("UPDATE ".DB::table('common_member_count')." SET $moneycredits=$moneycredits-{$_G['gp_banknum']} WHERE uid='$uid'");
                        DB::query("UPDATE ".DB::table('plugin_bankoperation')." SET opnum=opnum+{$mycuraccrual['all']}+{$_G['gp_banknum']},begintime='$timestamp' WHERE bankid='$bankid' AND uid='$uid' AND optype='0'");
			...
			if($bankinfo['bankroll']<$mycuraccrual['bank']) {
				showmessage($bankmsglang['bankroll_error']);
                    ...
                    DB::query("UPDATE ".DB::table('plugin_bankoperation')." SET opnum=opnum+{$mycuraccrual['all']},begintime='$timestamp' WHERE bankid='$bankid' AND uid='$uid' AND optype='0'");
		...
		if($op=='in') {
			...
			$endtime = $timestamp+$_G['gp_daynum']*86400;
			DB::query("UPDATE ".DB::table('common_member_count')." SET $moneycredits=$moneycredits-{$_G['gp_banknum']} WHERE uid='$uid'");
                        DB::query("INSERT INTO ".DB::table('plugin_bankoperation')."(uid,username,bankid,optype,opstatus,opnum,extchar,begintime,endtime) VALUES('$uid','$username','$bankid','1','0','{$_G['gp_banknum']}','{$bankinfo['fixedrate']}','$timestamp','$endtime')");
			...
			$query = DB::query("SELECT * FROM ".DB::table('plugin_bankoperation')." WHERE id='$noid' AND bankid='$bankid' AND uid='$uid' AND optype='1'");
			...
			if($bankpass=='') {
				...
				if($bankinfo['bankroll']<$myfixaccrual['bank']) {
					showmessage($bankmsglang['bankroll_error']);
                            ...
                            DB::query("UPDATE ".DB::table('common_member_count')." SET $moneycredits=$moneycredits+$banknum+{$myfixaccrual['all']} WHERE uid='$uid'");
			...
			$datanum = DB::result_first("SELECT COUNT(*) FROM ".DB::table('plugin_bankoperation')." WHERE bankid='$bankid' AND uid='$uid' AND optype='1'");
			if($datanum>0) {
				$multipage = multi($datanum, $pernum, $_G['gp_page'], "plugin.php?id=bank_ane:bank&bankid=$bankid&action=fix");
				$query = DB::query("SELECT * FROM ".DB::table('plugin_bankoperation')." WHERE bankid='$bankid' AND uid='$uid' AND optype='1' ORDER BY id DESC LIMIT $start_limit, $pernum");
				$rowcolor = 1;
				$datalist = array();
		...
		if($op=='ok') {
			...
			if($_G['gp_banknum']<10 || $_G['gp_banknum']+$mychgaccrual>$mycash) {//ttttttttttttmmmmmmmmmmmmmmmppppppppppppp
				showmessage($bankmsglang['banknum_error']);
                        ...
                        $touseresc = daddslashes($touser);
                        if($touser=='' || $touser==$username) {
                                showmessage($bankmsglang['user_error']);
                        }
                        $query = DB::query("SELECT * FROM ".DB::table('plugin_bankoperation')." WHERE bankid='$bankid' AND username='{$touseresc}' AND optype='0'");
			...
			DB::query("UPDATE ".DB::table('common_member_count')." SET $moneycredits=$moneycredits-{$_G['gp_banknum']}-$mychgaccrual WHERE uid='{$uid}'");
                    DB::query("UPDATE ".DB::table('plugin_bankoperation')." SET opnum=opnum+{$yourcuraccrual['all']}+{$_G['gp_banknum']},begintime='{$timestamp}' WHERE bankid='$bankid' AND uid='{$touserinfo['uid']}' AND optype='0'");
		...
		if($op=='try') {
			...
			if($_G['gp_daynum']<=0) {
				showmessage($bankmsglang['lendaynum_error']);
                        ...
                        DB::query("INSERT INTO ".DB::table('plugin_bankoperation')."(uid,username,bankid,optype,opstatus,opnum,extchar,begintime,endtime) VALUES('{$uid}','{$username}','$bankid','2','0','{$_G['gp_banknum']}','{$bankinfo['lendingrate']}','{$timestamp}','{$_G['gp_daynum']}')");
			...
			$noid = abs(intval(trim($_G['gp_noid'])));
			$query = DB::query("SELECT * FROM ".DB::table('plugin_bankoperation')." WHERE id='$noid' AND bankid='$bankid' AND uid='$uid' AND optype='2' AND opstatus='0'");
			...
			$noid = abs(intval(trim($_G['gp_noid'])));
			$query = DB::query("SELECT * FROM ".DB::table('plugin_bankoperation')." WHERE id='$noid' AND bankid='$bankid' AND uid='$uid' AND optype='2' AND opstatus='1'");
			if(!$leninfo = DB::fetch($query)) {
				showmessage($bankmsglang['var_error']);
			}
			...
			DB::query("UPDATE ".DB::table('common_member_count')." SET $moneycredits=$moneycredits-$banknum-$mylenaccrual WHERE uid='$uid'");
			...
			$query = DB::query("SELECT * FROM ".DB::table('plugin_bankoperation')." WHERE bankid='$bankid' AND uid='$uid' AND optype='2' ORDER BY opstatus DESC");
			$rowcolor = 1;
			$datalist = array();
			while($datashow = DB::fetch($query)) {
				$datashow['tr'] = $rowcolor++;
		...
		if($op=='cg') {
			...
			DB::query("UPDATE ".DB::table('plugin_bankoperation')." SET extchar='$newbankpass' WHERE bankid='$bankid' AND uid='$uid' AND optype='0'");
			...
			$query = DB::query("SELECT opnum FROM ".DB::table('plugin_bankoperation')." WHERE bankid='$bankid' AND uid='$uid' AND optype>'0'");
			...
			DB::query("DELETE FROM ".DB::table('plugin_bankoperation')." WHERE bankid='$bankid' AND uid='$uid'");
		...
		$moresql = ($show==1) ? "otheruser='{$username}'" : "uid='{$uid}'";
	...
	$query = DB::query("SELECT b.*,p.endtime FROM ".DB::table('plugin_banklist')." b LEFT JOIN ".DB::table('plugin_bankoperation')." p ON p.uid='$uid' AND p.optype='0' AND b.id=p.bankid");
	$banklist = array();
